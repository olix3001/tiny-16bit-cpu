...
yo_%:
	@mkdir -p build
	@echo "Synthesizing $*..."
	@yosys -p "read_verilog -sv src/$*.sv; \
	           hierarchy -check -top $*; \
	           proc; opt; fsm; opt; memory; opt; \
	           techmap; opt; clean; \
	           synth_ice40 -abc9 -top $*; \
			   ltp; \
	           stat" > build/$*_synth.log
	@echo "--- TIMING ESTIMATE (GATES) ---"
	@grep "Longest topological path" build/$*_synth.log
	@echo "--- RESOURCE USAGE ---"
	@grep -A 30 "Printing statistics" build/$*_synth.log

timing_%:
	mkdir -p build
	yosys -p "read_verilog -sv src/$*.sv; synth_ice40 -top $* -abc9 -json build/$*.json"
	nextpnr-ice40 --hx8k \
		--json build/$*.json \
		--asc build/$*.asc
	icetime -d hx8k -mtr build/$*.rpt build/$*.asc
	cat build/$*.rpt
